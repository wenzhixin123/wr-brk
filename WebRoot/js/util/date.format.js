 ...
 * 返回: 根据掩码格式化后的时间
 *  date 默认值是 当前日期时间
 *  mask(掩码) 默认值是 dateFormat.masks.default ,也就是这种格式 "ddd mmm dd yyyy HH:MM:ss"
 */
 ...
 * 使用方法和例子
	...
	now.format("m/dd/yy");	// Returns, 15/12/11
	...
	// 如果没有指定掩码,将使用默认掩码,返回格式如下
	now.format();
	// Thu Dec 15 2011 10:12:30
	
	// 如果dateFormat没有包含任何参数,
	// 则默认传入当前的时间和日期,并根据默认掩码格式化
	dateFormat();
	// Thu Dec 15 2011 10:12:30
	
	// 你可以不指定时间参数 (只要你的掩码不包含任何数字)
	//  在这种情况下,将使用当前日期和时间作为参数
	dateFormat("isoDateTime");
	// 2011-12-15T10:05:46
	
	// 最后, 你可以转换本地时间格式到 UTC 时间格式. 
	/ 或者传递一个可选的true参数(在这种情况下,所有参数都必须指定):
	dateFormat(now, "longTime", true);
	now.format("longTime", true);
	// Both lines return, e.g., 2:10:24 AM UTC
