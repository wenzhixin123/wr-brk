...
<script type="text/javascript">
	...
	function searchOrder(){
		...
		$("#logisticOrderGrid").datagrid("setQueryFields",[
      		{
      			fieldName:"transactionType",
      			fieldStringValue:"SOT"
      		},
      		{
		...
		$("#logisticOrderGrid").datagrid("commonQuery",{
	...
	function addOrder(){
		window.parent.addTabs("出库订单办理","<%=path%>/jsp/hw_ycl/order/inboundOrderEditYL.jsp?type=1",true,true);
	}
	function editOrder(){
		var selItem=check();
		if(selItem!=null){
				window.parent.addTabs("出库订单编辑","<%=path%>/jsp/hw_ycl/order/inboundOrderEditYL.jsp?submitOrderUuid="+selItem.submitOrderUuid,true,true);
...
<body class="easyui-layout">
	<div region="north" title="查询" border="false" iconCls="myCustomerIcon_searchForm">
		...
		<form id="searchForm" class="easyui-form" columns="4" i18nRoot="InOrderNoHeadYcl">
			<input name="submitOrderNo" id="submitOrderNo" title="出库订单号"  class="easyui-validatebox" onkeyup="strUppercase('#submitOrderNo')" onblur="strTrim('#submitOrderNo')"/>
			...
			<input name="cargoConsigneeCode" id="cargoConsigneeCode"  combo="true" class="easyui-combogrid" title="委托单位"  codetype="ALL_CUSTOMER"/>
			<input name="transactionStatus"  title="状态" id="transactionStatus" class="easyui-combobox"     codetype="STATUS" panelHeight="auto" />
		...
		<div class="easyui-tabs" fit="true" border="false" id="dataTabs">
			<div title="订单信息列表">
				<div class="easyui-layout" fit="true">
					<div region="north">
						<div class="datagrid-toolbar">
							<a class="easyui-linkbutton" plain="true"  iconCls="icon-add" onclick="addOrder();"> 增加 </a>
							<a class="easyui-linkbutton" id="editBtn" plain="true"  iconCls="icon-edit" onclick="editOrder();"> 编辑 </a>
							<a class="easyui-linkbutton" id="submitBtn" plain="true" iconCls="icon-ok" onclick="submitButton()"> 提交 </a>
					...
					<div region="center">
						<table class="easyui-datagrid" id="logisticOrderGrid" fit="true"  border="false" >
							<thead>
								<tr>
								    <th field="transactionStatus" title="状态" align="center" width="50px" codetype="STATUS"></th>
								     <th field="status" title="作业状态" align="center"  ></th>
								    <th field="logisticsOrderNo"  title="出库作业单号"></th>
								    <th field="orderNo" title="出库订单号"></th>
								    <!-- <th field="" title="入库订单类型"></th>	 -->	
								    <th field="orderNo" title="企业订单号"></th>
								    <!-- <th field="" title="企业订单类型"></th>	 -->							   								  
