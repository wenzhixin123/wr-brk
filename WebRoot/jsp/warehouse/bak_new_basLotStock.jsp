...
<script type="text/javascript">
	...
	//按钮权限控制
	var jurisdictionButtons=[{"btnCode":"PH_WARE_BAL_REMOVEWARE"},{"btnCode":"PH_STOCK_BAS_REMOVESTOCK"}];
	//是否选中了仓库区域
	var isSelectLot=false;
	//加载树形结构
	$(function(){
		__searchBaslLotStockDataChuShiHua();
		//获取权限
		_button_Jurisdiction(jurisdictionButtons,true);
		//仓库树形结构初始化、包括事件监听
		_treeChuShiHua();
		//初始化树形结构数据
		resultTree();
	...
	//按钮控制-屏蔽+释放
	function __buttonStateJump(value,cw,rowData){
		JURISD_BUT_NO=new Array();
		JURISD_BUT_YES=new Array();
		//默认全部关闭
		if("<%=CommonUtil.Active%>"==value){
			JURISD_BUT_NO.push("PH_STOCK_BAS_REMOVESTOCK");
		}else{
			_buttonAddJurisdiction("PH_STOCK_BAS_REMOVESTOCK");
		}
		//冻结
	...
	function removeWareLocArea(){
		var node = $('#warehouseTree').tree('getSelected');
		if(!is_null(node)&&node.attributes.data.remark!="wh"){
	...
	function addWareLocArea(){
		var node = $('#warehouseTree').tree('getSelected');
		if(!is_null(node)){
			var bwEntity=node.attributes.data;
	...
	function importFile(){
		...
		if(!is_null(node)){
			var bwEntity=node.attributes.data;
			if(bwEntity.remark=="wh"){
				var importFile=$('#importFile').sinotrans_Import({
					path:contextPath,
					fileType:'.xls,.xlsx',
					businessType:'YCL_AREA_IMPORT',
					modelIds:node.id,
		...
		if(!is_null(node)){
			...
			if(bwEntity.remark=="area"){
				...
				var importFile=$("#barcodeImport").sinotrans_Import({
					path:'<%=path%>',
					fileType:'.xls,.xlsx',
					businessType:'<%=CommonUtil.BAS_LOT_STOCK%>',
					modelIds:ld.join(","), //lodUUIDs.join(",")
					functionName:'callReturnback'
				});
				importFile.open();
			}else{
				$alertShow('请先选择区域！');
	...
	//打印功能
	function printLot(){
		var rows = $("#baslLotStockData").datagrid('getSelections');
		if(rows.length==0){
...
<script type="text/javascript">
	...
	function _treeChuShiHua(){
		...
		$("#warehouseTree").tree({
			onClick:function(node){
				//if(node.attributes.data.remark=="wh"){
				if(node.attributes != null && node.attributes.data != null){
					if(node.attributes.data.remark !="wh")
						return;
					isSelectLot=false;
					$("#jspTabs").tabs("select","仓库/区域信息");
					BasWarehouseManager.get(node.id,function(result){
						...
						$("#savesvgPath1").linkbutton("enable").unbind("click").bind("click");//显示上传地图按键
						//查询此仓库是否存在地图
						$.ajax({
							type: "POST",
							url: "<%=path%>/servlet/sinotransServlet?cmd=blob_findData",
							data: "jsonResult="+saveBasWarehouseUuid,
							success: function(datas){
								var databoolean="true";
								if(datas==databoolean){
									$("#mapmassege").html("该仓库地图存在！");
									$("#downloadmap").linkbutton("enable").unbind("click").bind("click"); 	//显示下载按键
									$("#isUpload").val(1);
								}else{
									$("#mapmassege").html("地图不存在，请上传！");
									$("#downloadmap").linkbutton("disable").unbind("click");               //灰掉下载按键
									$("#isUpload").val(0);
								}
							},
	...
	function uploadwarehousemap(){
		if(is_null(saveBasWarehouseUuid)){
			$alertShow(0);
			return;
		}
		var ld=[];
...
<div id="codeTypeContainer" region="west" title="仓库" border="true" style="padding:0px;width:300px;" split="true">
	...
	<div class="datagrid-toolbar">
		...
		<a href="#" id="PH_WARE_BAL_REMOVEWARE" class="easyui-linkbutton" onclick="removeWareLocArea()" iconCls="myCustomerIcon_remove">
			删除区域</a>
		<a href="#" id="import_excel" class="easyui-linkbutton" onclick="importFile()" iconCls="icon_shengcheng">
			导入区域</a>
	</div>
			...
			<div class="easyui-layout" border="true" fit="true">
				...
				<div region="center" title="仓库信息">
					...
					<table id="baslLotStockData" fit="true" class="easyui-datagrid" pageList="[30,50,100]">
						<thead>
						<tr>
							<th field="status" width="50px;"  title="状态" formatter="statusAtter"></th>
