  ...
  <head>
	...
	$(function(){
		$('#codeType_datagird').datagrid({
			...
			onClickRow:function(rowIndex,rowData){
				...
				var queryParams={};
				queryParams.basCodeTypeUuid = rowData.basCodeTypeUuid;
				queryParams.status=$('#status').combobox('getValue');
				queryParams.modifiable=$('#modifiable').combobox('getValue');
				$("#codeDef_datagird").datagrid("load",queryParams);
	...
	$(function(){
		...
		$('#searchBtn').click(function(){ 
			var queryParams={};
			queryParams.codeValue=$('#codeValue').val();
			queryParams.status=$('#status').combobox('getValue');
			queryParams.codeNumber=$('#codeNumber').val();
			queryParams.modifiable=$('#modifiable').combobox('getValue');
            $("#codeDef_datagird").datagrid('load',queryParams);
    	});
				...
				$.messager.confirm('确定框', '确定设置这条记录为默认吗?',function(r){
					if(r){
						...
						if(_dRow){
							...
							_dRow.rownum=undefined;
							_dRow.uuid=undefined;
						...
						currentRow.rownum=undefined;
						currentRow.uuid=undefined;
	...
	$(function(){
		...
		function fn_toolbar_saveBtn(){
			...
			if (updateRows.length>0) {
				for ( var i = 0; i < updateRows.length; i++) {
					...
					updateRows[i].rownum=undefined;
					updateRows[i].uuid=undefined;
			...
			if (deleteRows.length>0) {
					for ( var j = 0; j < deleteRows.length; j++) {
						...
						deleteRows[j].rownum=undefined;
						deleteRows[j].uuid=undefined;
		...
		function fn_toolbar_validateBtn(){
			...
			if(rows && rows[0].basCodeDefUuid==undefined){
			    alert("请先保存数据!")
				return ;
			}
				...
				$.messager.confirm('确定框', '确定生效'+rows.length+'条记录吗?', function(r){
					if(r){
						$.each(rows,function(i,v){
							...
							selectedRow.rownum=undefined;
							selectedRow.uuid=undefined;
			...
			<form id="queryForm">
				<table id="searForm" width="880px" style="margin:0;padding:0;">
					<tr>
						...
						<td>
							<input id="codeValue" name="codeValue"  class="easyui-validatebox" />
						</td>
						<td align="center" td_align="left" class="textStyle">
							<label for="status" >
					...
					<tr>
						...
						<td>
							<input id="codeNumber" name="codeNumber" class="easyui-validatebox" />
						</td>
	  	...
	  	<div class="easyui-layout" fit="true" border="false">
			<div id="codeTypeContainer" region="west" title="数据词典类型" border="true" style="width:460px">
					<table id="codeType_datagird" iconCls="icon-edit" fit="true"
								url="<%=request.getContextPath()%>/servlet/SinotransServlet?cmd=codeTypeSpecial">
					</table>
			</div>
			<div id="codeDefContainer"  region="center" title="数据词典定义" border="true" >
				<table id="codeDef_datagird" iconCls="icon-edit" fit="true"
					url="<%=request.getContextPath()%>/servlet/SinotransServlet?cmd=codeDefSpecial">
					<thead>
						<tr>
