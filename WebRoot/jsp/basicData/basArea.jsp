...
<script type="text/javascript">
	...
	function save() {
		...
		BasAreaManager.saveAll(rows, function(result) {
	...
	$(function() {
		...
		$("#gridResult").datagrid("options").onBeginEdit = function(rowIndex, rowData) {
			...
			$("#gridResult").datagrid("getColumnEditor", "areaCode").select();
			
			//自动填写城市名称
			$("#gridResult").datagrid("getColumnEditor", "cityCode").combo("options").onHidePanel = function () {
				$.fn.combogrid.defaults.onHidePanel.apply(this);
				$("#gridResult").datagrid("getColumnEditor", "cityName")[0].value = $(this).combo("getText");
			};
		...
		//弹出窗口自动填写城市名称
		$("#cityCode").combogrid("options").onHidePanel = function (value) {
			$("#cityName").val($("#cityCode").combogrid("getText"));
		};
  ...
  <body class="easyui-layout">
  	<div region="north" title="查询条件" iconCls="icon-search">
		...
		<form id="formQuery" class="easyui-form" columns="3" i18nRoot="BasArea">
			<input name="areaCode"/>
			<input name="areaName" operator="ilikeAnywhere"/>
			<input name="areaNameEn"" operator="ilikeAnywhere"/>
			<input name="cityName" operator="ilikeAnywhere"/>
			<input name="cityNameEn"/>
			<input name="status" class="easyui-combobox" editable="false" codetype="STATUS"/>
		</form>
	</div>
	
	<div region="center" title="区县信息" iconCls="icon-edit">
		<div class="datagrid-toolbar">
		...
		<table id="gridResult" class="easyui-datagrid" i18nRoot="BasArea" fit="true">
			<thead>
				<tr>
					<th field="areaCode" editor="{type:'validatebox', options:{required:true, validType:'length[0,50]'}}"/>
					<th field="areaName" editor="{type:'validatebox', options:{required:true, validType:'length[0,150]'}}"/>
					<th field="areaNameEn" editor="{type:'validatebox', options:{validType:'length[0,150]'}}"/>
					<th field="cityCode" editor="{type:'combogrid', options:{required:true}}" codetype="ALL_CITY"/>
					<th field="cityName" editor="{type:'validatebox', options:{validType:'length[0,150]'}}"/>
					<th field="cityNameEn" editor="{type:'validatebox', options:{validType:'length[0,150]'}}"/>
					...
					<th field="districtNum"" editor="{type:'validatebox', options:{validType:'length[0,50]'}}"/>
					...
					<th field="centerCode" editor="{type:'validatebox', options:{validType:'length[0,150]'}}"/>
				</tr>
			</thead>
		</table>
	</div>
	...
	<!-- 弹出窗口编辑 -->
	<div id="dialogEdit" class="easyui-dialog" title="区县信息" iconCls="icon-edit"
			style="width:600px;padding:10px" closed="true" modal="true">
		<form id="formEdit" class="easyui-form" columns="2" i18nRoot="BasArea">
			<input name="areaCode" valiType="length[0,50]" required="true"/>
			<input name="areaName" class="easyui-validatebox" valiType="length[0,150]" required="true"/>
			<input name="areaNameEn"" class="easyui-validatebox" valiType="length[0,150]"/>		
		    <input id="cityCode" name="cityCode" class="easyui-combogrid" codetype="ALL_CITY" required="true" onchange="autoGetCityName(this.value)"/>
			<input id="cityName" name="cityName" class="easyui-validatebox" valiType="length[0,150]"/>
