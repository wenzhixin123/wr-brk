...
<script type="text/javascript">
	...
	function appendRow() {
		...
		if (parent.getTabIframe("订单录入").contentWindow.reset) {
			parent.getTabIframe("订单录入").contentWindow.reset();
		}
	};
	...
	function editRow() {
		...
		if (selected) {
			parent.addTab("订单录入", "jsp/order/orderEdit.jsp", "icon icon-nav", true);
			if (parent.getTabIframe("订单录入").contentWindow.loadByOrderNo) {
				parent.getTabIframe("订单录入").contentWindow.loadByOrderNo(selected.logisticsOrderNo);
			} else {
				parent.getTabIframe("订单录入").contentWindow.orderNo = selected.logisticsOrderNo;
			}
		}
	};
		...
		<table id="gridResult" class="easyui-datagrid" i18nRoot="LogisticsOrder" fit="true">
			<thead>
				<tr>
					...
					<th field="consignee" codetype="CIM_CUST"/>
