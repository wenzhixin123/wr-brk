...
<script type="text/javascript">
	...
	function appendRow() {
		...
		if (parent.getTabIframe("订单录入-简版").contentWindow.reset) {
			parent.getTabIframe("订单录入-简版").contentWindow.reset();
		}
	};
	...
	function editRow() {
		...
		if (selected) {
			parent.addTab("订单录入-简版", "jsp/order_simple/orderEdit_simple.jsp", "icon icon-nav", true);
			if (parent.getTabIframe("订单录入-简版").contentWindow.loadByOrderNo) {
				parent.getTabIframe("订单录入-简版").contentWindow.loadByOrderNo(selected.logisticsOrderNo);
			} else {
				parent.getTabIframe("订单录入-简版").contentWindow.orderNo = selected.logisticsOrderNo;
			}
		}
	};
		...
		<form id="formQuery" class="easyui-form">
			<table>
				...
				<tr>
					...
					<td>
						<input name="consignee" class="easyui-combogrid" codetype="CIM_CUST"/>
					</td>
		...
		<table id="gridResult" class="easyui-datagrid" i18nRoot="LogisticsOrder" fit="true" border="false">
			<thead>
				<tr>
					...
					<th field="consignee" codetype="CIM_CUST" width="100"/>
