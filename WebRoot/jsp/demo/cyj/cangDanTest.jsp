...
function openWarehousingDialog(){
	$("#warehousingUploadDialog").dialog("open");
}

function closeWarehousingDialog(){
	$("#warehousingUploadDialog").dialog("close");
}

function uploadWarehosingFile(){
	var warehousingType = $("#warehousingType").val();
	var fileName = $("#uploadWarehousingFile").val();
	var fileType = fileName.substring(fileName.lastIndexOf('.') + 1, fileName.length);
	var uuid = generateUUID();
	
	$("#formWarehousingUpload").form("submit",{
	    url:contextPath+"/JsonFacadeServlet?uuid=" + uuid + "&type=" + fileType, 
	    onSubmit: function(){
	    	if (!$("#formWarehousingUpload").form("validate")) {
	    		$.messager.alert("提示", "舱单入仓表单域数据验证错误！", "warning");
	    		return false;
	    	}
	    	if(fileName == ""){
	    		$.messager.alert("提示", "请选择导入文件！", "warning");
	    		return false;
	    	}
	    	if(warehousingType == "excel"){
	    		if(fileType.toUpperCase() != "XLS" && fileType.toUpperCase() != "XLSX"){
		    		$.messager.alert("提示", "文件类型不正确！", "error");
		    		return false;
		    	}
	    	}
	    	if(warehousingType == "transport" || warehousingType == "southPlatform"){
	    ...
	    success:function(data){
	    	var warehousingPortAreaCode = $("#warehousingPortAreaCode").combogrid("getValue");
	    	var warehousingCntrAdminCode = $("#warehousingCntrAdminCode").combogrid("getValue");
	    	var warehousingLinerCode = $("#warehousingLinerCode").combogrid("getValue");
	    	DcsWarehousingManager.uploadWarehousingFile(uuid, warehousingPortAreaCode, warehousingCntrAdminCode, warehousingLinerCode, warehousingType, fileType, fileName, function(result) {
	    		//alert(result.result);
...
<body class="easyui-layout">
	<div region="north" title="舱单" >
		...
		<span class="panel-header panel-title" style="float: left; border-style: none; width: 100px;">舱单查询条件</span>
			...
			<a class="easyui-linkbutton" iconCls="icon-add" onclick="openWarehousingDialog()" key="W">进口舱单导入</a>
		</div>
...
<!-- 校验 -->
	...
	<!-- 进口舱单导入窗口 -->
	<div id="warehousingUploadDialog" class="easyui-dialog" title="进口舱单导入" style="width:600px;padding:10px" closed="true" modal="true" iconCls="icon-edit">
		<form id="formWarehousingUpload" method="post" enctype="MULTIPART/FORM-DATA"  class="easyui-form" columns="1">
			<select name="warehousingType" id="warehousingType" title="文件类型">
				<option value="excel" selected="selected">EXCEL</option>
				<option value="transport">交通部EDI格式</option>		
				<option value="southPlatform">南方平台格式</option>
			</select>
			<input id="warehousingPortAreaCode" name="warehousingPortAreaCode" title="舱单签发地" class="easyui-combogrid" required="true" validType="length[0,10]" codetype="PORT_AREA_CODE"/>
			<input id="warehousingCntrAdminCode" name="warehousingCntrAdminCode" title="控箱公司" class="easyui-combogrid" codetype="ALL_CNTR_ADMIN" required="true"/>
			<input id="warehousingLinerCode" name="warehousingLinerCode" title="船公司" class="easyui-combogrid" required="true" codetype="ALL_CARRIAGE"/>
			<input type="file" id="uploadWarehousingFile" name="浏览" size="50"/>
			<ul style="list-style-type:none">
				<li><a class="easyui-linkbutton" style="float:right; margin:10px; margin-right:70px" iconCls="icon-cancel" onclick="closeWarehousingDialog()" key="Q">取消</a></li>
				<li><a class="easyui-linkbutton" style="float:right; margin:10px;" iconCls="icon-ok" onclick="uploadWarehosingFile()" key="R">确认</a></li>
			</ul>
		</form>
	</div>
