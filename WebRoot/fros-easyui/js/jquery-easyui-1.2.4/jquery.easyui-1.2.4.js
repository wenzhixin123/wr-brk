...
(function($) {
	...
	function resetGridEvent(target) {
		...
		//wcj: use delegate to fasten
		gridBody.undelegate('tr[datagrid-row-index]', '.datagrid').delegate('tr[datagrid-row-index]',
				...
				}).delegate('tr[datagrid-row-index]', 'mouseleave.datagrid', function() {
			var rowIndex = $(this).attr('datagrid-row-index');
			gridBody.find('tr[datagrid-row-index=' + rowIndex + ']')
					.removeClass('datagrid-row-over');
		}).undelegate('tr[datagrid-row-index]', 'click.datagrid').delegate('tr[datagrid-row-index]', 'click.datagrid', function() {
					var rowIndex = $(this).attr('datagrid-row-index');
				...
				}).delegate('tr[datagrid-row-index]', 'dblclick.datagrid', function() {
					var rowIndex = $(this).attr('datagrid-row-index');
					if (opts.onDblClickRow) {
						opts.onDblClickRow.call(target, rowIndex, data.rows[rowIndex]);
					}
				}).delegate('tr[datagrid-row-index]', 'contextmenu.datagrid', function(e) {
					var rowIndex = $(this).attr('datagrid-row-index');
					if (opts.onRowContextMenu) {
						opts.onRowContextMenu.call(target, e, rowIndex, data.rows[rowIndex]);
					}
				});
		gridBody.undelegate('td[field]', '.datagrid').delegate('td[field]', 'click.datagrid',
				function() {
				...
				}).delegate('td[field]', 'dblclick.datagrid', function() {
					var rowIndex = $(this).parent().attr('datagrid-row-index');
					var field = $(this).attr('field');
					var value = data.rows[rowIndex][field];
					opts.onDblClickCell.call(target, rowIndex, field, value);
				});
		gridBody.undelegate('div.datagrid-cell-check input[type=checkbox]', '.datagrid')
		.delegate('div.datagrid-cell-check input[type=checkbox]', 'click.datagrid', function(e) {
...
//				});
	...
	function renderGrid(target, data) {
		...
		//wcj: row has fixed row height, no need to fix
		//fixRowHeight(target);
		...
		render : function(target, container, frozen) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				var style = opts.rowStyler ? opts.rowStyler.call(target, i,
						rows[i]) : '';
				//wcj: fixed row height
				//style = style ? 'style="' + style + '"' : '';
				style = 'style="' + (style || '') + 'height:22px;"';
