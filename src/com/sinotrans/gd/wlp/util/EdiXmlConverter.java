...
public class EdiXmlConverter {
	...
	private boolean elementNameFirstLowered = true;
	private boolean unFullElement = true;
	...
	@SuppressWarnings("unchecked")
	private Object parseXmlElement(Class<?> clazz, Type genericType,
				...
				for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
					...
					if (beanWrapper.isWritableProperty(propertyName)) {
						...
						if (convertElementName) {
							elementName = SqlUtils
									.javaNameToDbName(elementName);
							if (!elementNameUppered) {
								elementName = elementName.toLowerCase();
							}
						}
						if(!elementNameFirstLowered){
							elementName = StringUtil.replaceCharAt(elementName,0,elementName.substring(0,1).toUpperCase());
						}
	...
	public String buildXmlString(Object object) throws Exception {
		...
		if(!unFullElement){
			returnString = StringUtils.replace(returnString, ">null<", "><");
		}
		return returnString;
	}
	...
	private Element buildXmlElement(String name, Object value) throws Exception {
		...
		if(!elementNameFirstLowered){
			name = StringUtil.replaceCharAt(name,0,name.substring(0,1).toUpperCase());
		}
	...
	public boolean isElementNameFirstLowered() {
		return elementNameFirstLowered;
	}
	 ...
	 * @param elementNameUppered
	 */
	public void setElementNameFirstLowered(boolean elementNameFirstLowered) {
		this.elementNameFirstLowered = elementNameFirstLowered;
	}

	public boolean isUnFullElement() {
		return unFullElement;
	}
	 ...
	 * @param elementNameUppered
	 */
	public void setUnFullElement(boolean unFullElement) {
		this.unFullElement = unFullElement;
	}
