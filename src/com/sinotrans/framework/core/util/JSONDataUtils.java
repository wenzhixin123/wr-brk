...
public class JSONDataUtils {
	...
	public static Object parseJSONValue(Class<?> clazz, Type genericType, Object jsonValue) throws Exception {
		...
		if (genericType instanceof GenericArrayType) {
			...
			if (typeClass.isArray()) {
				...
				if (jsonValue instanceof String) {
					...
					return parseJSONValue(clazz, Object[].class, jsonValue);
	...
	public static String buildJSONString(Object value) throws Exception {
		return buildJSONValue1(value).toString();
	}
	
	public static Object buildJSONValue1(Object value) throws Exception {
		if (value == null) {
			return JSONObject.NULL;
		} else if (value.getClass().isArray()) {
			JSONArray jsonArray = new JSONArray();
			for (int i = 0; i < Array.getLength(value); i++) {
				jsonArray.put(buildJSONValue1(Array.get(value, i)));
			}
			return jsonArray;
			...
			for (Object item : (Collection<?>) value) {
				jsonArray.put(buildJSONValue1(item));
			}
			return jsonArray;
			...
			for (Object key : map.keySet()) {
				jsonMap.put(key.toString(), buildJSONValue1(map.get(key)));
			}
			return jsonMap;
			...
			for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
				...
				if (beanWrapper.isReadableProperty(propertyName)) {
					...
					jsonObject.put(propertyName, buildJSONValue1(propertyValue));
				}
			}
			return jsonObject;
		}
	...
	public static Object buildJSONValue(Object value) throws Exception {
		if (value == null) {
			...
			Map<?, ?> map = (Map<?, ?>) value;
			for (Object key : map.keySet()) {
				jsonMap.put(key.toString(), buildJSONValue(map.get(key)));
			}
			return jsonMap;
		} else if (value instanceof String
				|| value instanceof Integer
